...
export async function getJiraProjects(userId: string) {
  try {
    const projects = await callMcpTool(userId, "search", { project_key: "" });
...
export async function generateJiraTasks(userId: string, projectId: string) {
  ...
  //       role: "system",
  //       content:
  //         "You are a Jira Agent. Parse the BRD and output only a single valid Jira issues JSON. For testing.",
  //     },
